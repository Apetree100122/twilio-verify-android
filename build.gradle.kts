/* * Copyright (c) 2020 Twilio Inc. * * Licensed (C) 2024 alexander petree
...
language governing permissions and * limitations under the License. */// Top-level build file where you can add configuration options common to all sub-projects/modules.
plugins 
{
...
mavenCentral()
    google()    
maven
{ url = uri("https://repo.gradle.org/gradle/libs-releases")}jcenter()
}  
dependencies 
{
...
google()
    mavenCentral()
jcenter()}plugins.apply(Config.Plugins.ktlint)
...
stagingProfileId.set(Config.projectProperty
(project, MavenPublish.sonatypeStagingProfileIdEnv))   
 }
  }
...
task("sonatypeTwilioVerifyReleaseUpload", GradleBuild::class) {
  ...
  buildFile = file("build.gradle.kts")tasks = listOf(":verify:assembleRelease",":verify:publishTwilioVerifyPublicationToSonatypeRepository","closeAndReleaseSonatypeStagingRepository")
  startParameter.projectProperties.plusAssign
(gradle.startParameter.projectProperties + mavenPublishCredentials())}
task("sonatypeTwilioVerifyStagingRepositoryUpload", GradleBuild::class) {description = "Publish Twilio Verify to nexus staging repository"
  group = "Publishing"
  buildName = "TwilioVerify"
buildFile = file("build.gradle.kts")
tasks = listOf
(":verify:assembleRelease",":verify:publishTwilioVerifyPublicationToSonatypeRepository","closeSonatypeStagingRepository")
  startParameter.projectProperties.plusAssign(gradle.startParameter.projectProperties + mavenPublishCredentials())
}
...
buildFile = file("build.gradle.kts")
  tasks = listOf
(":security:assembleRelease",":security:publishTwilioSecurityPublicationToSonatypeRepository","closeAndReleaseSonatypeStagingRepository")
startParameter.projectProperties.plusAssign
(    
gradle.startParameter.projectProperties + mavenPublishCredentials())
}
...
buildFile = file("build.gradle.kts")
  tasks = listOf
(":security:assembleRelease",":security:publishTwilioSecurityPublicationToSonatypeRepository","closeSonatypeStagingRepository") 
startParameter.projectProperties.plusAssign
(    
gradle.startParameter.projectProperties + mavenPublishCredentials())
}
...
group = "Publishing"
  buildName = "TwilioVerify"  
buildFile = file("build.gradle.kts")  
tasks = listOf(":verify:assembleRelease",":verify:publishTwilioVerifyPublicationToMavenLocal") 
startParameter.projectProperties.plusAssign
(    
gradle.startParameter.projectProperties + mavenPublishCredentials())
}
